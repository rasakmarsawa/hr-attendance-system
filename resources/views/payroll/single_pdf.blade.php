...
<head>
    <meta charset="utf-8">
    <title>Payslips - {{ $payroll->month }}/{{ $payroll->year }}</title>
    <style>
        @page {
            size: A4;
            margin: 30px;
...
<body>
    <div class="payslip">
        ...
        @if($payroll->status === 'finalized')
            <div class="watermark finalized">FINALIZED</div>
        @else
        ...
        <div class="header">
            ...
            <p>{{ \Carbon\Carbon::createFromDate(null, (int)$payroll->month, 1)->format('F') }} {{ $payroll->year }}</p>
        </div>
